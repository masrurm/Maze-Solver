...
void generate_all_paths( maze_cell ** maze, int dimension, int row, int column, char * path )
{
    ...
    if ( // INSERT CODE HERE: Simply return if we hit one of the base cases (there are more than 1)
        ...
        (row > dimension - 1) || (row < 0) || 
		(column > dimension - 1) || 
		(column < 0) || maze[row][column].character == '*' || maze[row][column].visited == VISITED ) {
        return;
	}
...
void display_cheapest_path()
{
	...
	// INSERT CODE HERE
	for (i = 0; i < paths_found; i++) {
		if (min_cost > path_cost(paths[i])) {
			cheapest_path = i;
			min_cost = path_cost(paths[i]);
		}
	}
